...
void Heap<E>::siftDown(int pos) {
    ...
    while (!isLeaf(pos)) {
        ...
        // rc 右子节点可能不存在需要检验
        if (rc < num && compare->prior(heap[rc], heap[lc])) lc = rc;
        // 此时lc指向左右节点大的一个
        if (compare->prior(heap[pos], heap[lc])) return;
        swap(heap, pos, lc);
        pos = lc;
    }
}
...
void Heap<E>::insert(const E &item) {
    ...
    heap[curr] = item;
    // 一直往上走
    while (curr != 0 && compare->prior(heap[curr], heap[parent(curr)])) {
...
E Heap<E>::remove(int pos) {
    ...
    if (pos == num - 1) num --;
        ...
        swap(heap, pos, --num);
        // 先上走
        while (pos != 0 && compare->prior(heap[pos], heap[parent(pos)])) {
